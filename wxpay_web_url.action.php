    ...
    public function houtai(){
		...
		if ( $check ) {
			...
			if ($open_activity == 1) {
				...
				$atime = time();
				$dtime = strtotime(date("Y-m-d"));//当天0点
				$etime = strtotime(date('Ymd')) + 86400;//当天24点
				$big_money = 10;
				$z_money = 50;
				if ($atime > 1513008000 || $uid == 1 || $uid == 4) {
					if ($total_fee_t > $big_money || $total_fee_t == $big_money) {

						$c_re = $this->db->GetList("select * from `@#_member_addmoney_record` where `pay_type`='微信公众号' and `status`='已付款' and `uid` = ".$uid." and `time` > '$dtime' ");

						$useractivity = $this->db->GetOne("select * from `@#_member_activity_money` where `uid` = ".$uid );

						//每日首充送50
						if (count($c_re) == 1) {
							if (empty($useractivity)) {
								$this->db->Query("INSERT INTO `@#_member_activity_money` (`uid`, `money`, `time`, `status`) VALUES ('$uid', '$z_money', '$atime', '0')");
							}else{
								$this->db->Query("UPDATE `@#_member_activity_money` SET `money` = `money` + '$z_money'  WHERE `uid` = ".$uid);
							}		
						}

						$useractivity1 = $this->db->GetOne("select * from `@#_member_activity_money` where `uid` = ".$uid );
        				if (!empty($useractivity1)) {
            				if ($useractivity1['money'] > 0 ) {

								$this->db->Query("UPDATE `@#_member_activity_money` SET `money` = `money` - '10'  WHERE `uid` = ".$uid);

								$this->db->Query("INSERT INTO `@#_member_activity_money_history` (`uid`, `money`, `info`,`type`,`time` ) VALUES ('$uid', '10','扣除赠送金额10元','1','$atime')");

								$this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '充值活动赠送金额10元', '10', '$atime')");

	                			$this->db->Query("UPDATE `@#_member` SET `money` = `money` + '10'  WHERE `uid` = ".$uid);
