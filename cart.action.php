...
$shoparr = $this->db->GetList ( "SELECT * FROM `@#_shoplist` where `id` in($shopids)", array ("key" => "id" ) );}
	  ...
	  public function cartlist() {
		  ...
		  //var_dump($Mcartlist);
		  if ($shopids != NULL) {
$shoparr = $this->db->GetList ( "SELECT `money`,`title`,`thumb`,`q_end_time`,`shenyurenshu`,`id`,`yunjiage`,`zongrenshu`,`canyurenshu` FROM `@#_shoplist` where `id` in($shopids)", array ("key" => "id" ));}
		  if (! empty ( $shoparr )) {
			  foreach ( $shoparr as $key => $val ) {
	...
	// 支付界面2
	  ...
	  public function pay1() {
		  ...
		  session_start ();
		  $_SESSION ['submitcode'] = $submitcode = uniqid ();
		  include templates ( "mobile/cart", "payment1" );
	  }
	//支付界面
